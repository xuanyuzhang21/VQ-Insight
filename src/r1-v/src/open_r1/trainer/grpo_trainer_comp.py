...
class Qwen2VLGRPOTrainerComp(Trainer):
    """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs[0]['data_type'] == 'video' and inputs[0]['data_source'] == 'video_fusion':
            input_copy[0]['content'][1]['video'] = inputs[0]['path1']
            input_copy[0]['content'][3]['video'] = inputs[0]['path2']
        
        elif inputs[0]['data_type'] == 'video':
            input_copy[0]['content'][0]['video'] = inputs[0]['path']

        elif inputs[0]['data_type'] == 'image':
            input_copy[0]['content'][1]['image'] = inputs[0]['path1']
            input_copy[0]['content'][3]['image'] = inputs[0]['path2']  
        ...
        if self.temporal and video_inputs:
            if len(video_inputs) == 2:
                ...
                indices1 = torch.randperm(video_inputs[1].size(0))
                shuffled_video_inputs = [video_inputs[0][indices], video_inputs[1][indices1]]
